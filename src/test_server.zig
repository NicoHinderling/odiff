...
test "server: different dimensions detected as pixel diff" {
    var gpa = std.heap.GeneralPurposeAllocator(.{}){};
    defer _ = gpa.deinit();
    const allocator = gpa.allocator();
    ...
    const request =
        \\{"requestId":20,"base":"test/png/white4x4.png","compare":"test/png/purple8x8.png","output":"/tmp/test_layout_diff.png","options":{"threshold":0.1}}
        \\
    ;
    try stdin.writeAll(request);
